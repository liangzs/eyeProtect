...
//    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case ACTION_UP:
                if (!isMove) {
                    clickIndex = checkWhichPoint(startX, startY);
                    startAmination();
                }
                break;
        }
...
//        anim.setInterpolator(new BounceInterpolator());
        ...
        anim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                //更新curPoint，即更新当前坐标
                animCirclePoint = (CirclePoint) animation.getAnimatedValue();
                // 刷新，重现调用onDraw()方法
...
//        anim.setInterpolator(new BounceInterpolator());
     ...
     * @return
     */
    private int checkWhichPoint(float x, float y) {
        for (int i = 0; i < circlePoints.size(); i++) {
            if (x > circlePoints.get(i).getP().X - circleRadius - 5 && x < circlePoints.get(i).getP().X + 5 + circleRadius
                    && y > circlePoints.get(i).getP().Y - 5 - circleRadius && y < circlePoints.get(i).getP().Y + 5 + circleRadius) {
                return i;
            }
        }
        return 0;
